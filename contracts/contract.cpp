...
CONTRACT mycontract : public contract {
   public:
    ...
    bool decode_base58(const string& str, vector<unsigned char>& vch) { return DecodeBase58(str.c_str(), vch); }
    ...
    pair<name, authority> parsememo(string memo) {
        ...
        authority auth{.threshold = 1, .keys = {k}, .accounts = {}, .waits = {}};
    ...
    void on_eos(name from, name to, asset quantity, string memo) {
        ...
        newaccount new_account = newaccount{.creator = _self, .name = account, .owner = auth, .active = auth};

        action(permission_level{_self, "active"_n}, name("eosio"), name("newaccount"), new_account).send();
        ...
        action(permission_level{_self, "active"_n}, name("eosio"), name("buyrambytes"), make_tuple(_self, _self, ACCOUNT_COST + OPEN_TOKEN_COST)).send();

        action(permission_level{_self, "active"_n}, name("eosio"), name("ramtransfer"), make_tuple(_self, account, ACCOUNT_COST, std::string("Account creation"))).send();

        action(permission_level{_self, "active"_n}, name("eosio.token"), name("open"), make_tuple(account, S_SYS, _self)).send();

        asset excess = quantity - minimumCost;
        if (excess.amount > 0) {
            action(permission_level{_self, "active"_n}, name("eosio.token"), name("transfer"), make_tuple(_self, account, excess, string("Overspent on account creation"))).send();
        }

        // creation date
        auto now = current_time_point().time_since_epoch().count();

        action(permission_level{_self, "active"_n}, _self, name("logcreation"), make_tuple(account, excess, minimumCost, now)).send();
    }
    ...
    void on_vaulta(name from, name to, asset quantity, string memo) {
        ...
        action(permission_level{_self, "active"_n}, name("core.vaulta"), name("newaccount"), new_account).send();
        ...
        // transfer ram to new account
        action(permission_level{_self, "active"_n}, name("core.vaulta"), name("buyrambytes"), make_tuple(_self, _self, ACCOUNT_COST + OPEN_TOKEN_COST)).send();

        action(permission_level{_self, "active"_n}, name("core.vaulta"), name("ramtransfer"), make_tuple(_self, account, ACCOUNT_COST, std::string("Account creation"))).send();

        asset excess = quantity - minimumCost;
        if (excess.amount > 0) {
            action(permission_level{_self, "active"_n}, name("core.vaulta"), name("transfer"), make_tuple(_self, account, excess, string("Overspent on account creation"))).send();
        }
    ...
    [[eosio::action]] asset estimatecost() { return getRamCost(ACCOUNT_COST + OPEN_TOKEN_COST, S_SYS); }

    ACTION logcreation(name account, asset excess, asset ram, uint64_t timestamp) { require_auth(_self); }
};
